...
/* block for resume event when suspend is running */
bool check_running_suspend;
#endif
...
#ifdef CONFIG_HSIC_EURONLY_APPLY
        /* check_running_suspend : block for resume event when suspend is running */
        if (pm_data->gpio_host_ready) {
                check_running_suspend = true;
                pr_info("dev rdy val = %d, suspend = %d\n",
                        gpio_get_value(pm_data->gpio_device_ready), check_running_suspend);
