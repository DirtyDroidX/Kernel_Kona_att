...
{
	pr_info("%s: Reseting the mdm due to AP request %d,%d\n", __func__,force_dump,mdm_drv->mdm_ready);

	if ( force_dump || (!mdm_drv->mdm_ready) )
	return;
...
{
	pr_info("%s:sim_polarity = %s\n", __func__,
			mdm_drv->pdata->sim_polarity == 1 ? "High" : "Low");
	if (mdm_drv) {
		mdm_drv->sim_state =
