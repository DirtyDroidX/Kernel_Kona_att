...
int count_remote_wakeup = 0;
int request_mdm_force_fatal = 0;
#endif
...
#ifdef CONFIG_HSIC_EURONLY_APPLY
	dev_dbg(&intf->dev, "count_remote_wakeup value to set 0\n");
	count_remote_wakeup = 0;
#endif
...
int usb_port_suspend(struct usb_device *udev, pm_message_t msg)
{
	...
	if (udev->do_remote_wakeup) {
		...
		if (status) {
			...
			if (status == -71)
			{
				if (count_remote_wakeup>10)
				{
					panic("won't remote wakeup -71! 10 times");
					dev_dbg(&udev->dev, "won't remote wakeup status = -71 happened 10TIMES !!! \n");
					count_remote_wakeup = 0;
					request_mdm_force_fatal = 1;
//					mdm_force_fatal();
				}
				else
				{
