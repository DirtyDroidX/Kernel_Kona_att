		...
		const isx012_regset_t regs[], int size, const char *name)
{
	...
	while ( i < size )//0<1
	{
		...
		else
		{
				...
				CHECK_ERR_COND_MSG(err <= 0, -EIO, "burst write i2c fail\n\n");
				iTxDataIndex = 0;
		}
		i++;
	}
...
static int isx012_post_poweron(struct v4l2_subdev *sd)
{
	...
	CHECK_ERR_MSG(err, "isx012 om_change fail!\n");

	/* Pre-Sleep */
	cam_info("=== Bootup: pre-sleep mode ===\n");
	isx012_set_from_table(sd, "set_pll_4",
		&state->regs->set_pll_4, 1, 0);
	msleep_debug(10, false);
	err = isx012_is_om_changed(sd);
	CHECK_ERR_MSG(err, "isx012 Pre-Sleep fail!\n");

	/* Sleep */
	cam_info("=== Bootup: sleep mode ===\n");
	isx012_writeb(sd, 0x00BF, 0x01);
	/* Negate nSTBY pin */
	...
	err = isx012_is_om_changed(sd);
	CHECK_ERR_MSG(err, "isx012 Sleep fail!\n");

	/* Active */
	cam_dbg("=== Bootup: active mode ===\n");
	err = isx012_is_cm_changed(sd);
	CHECK_ERR_MSG(err, "isx012 Active fail!\n");
